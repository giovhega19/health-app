/**
 * `ReorderRoutineItems` — pruebas adicionales de cobertura (Art. 3.2) para
 * las ramas de error que `ReorderRoutineItems.test.ts` (QA) no ejercita.
 */
...
describe("ReorderRoutineItems — ramas de error", () => {
  ...
  it("propaga el error del repositorio al buscar la rutina", async () => {
    ...
    const useCase = new ReorderRoutineItems(failing);
  ...
  it("propaga el error de Routine.reorderItems cuando el bloque no existe", async () => {
  ...
  it("propaga el error del repositorio al guardar", async () => {
    ...
    const result = await useCase.execute({ routineId: routine.id, blockId: block.id, orderedItemIds: [item.id] });
