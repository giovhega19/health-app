...
| T19 | ☐ | Abrir PR de scaffold y verificar que los 3 jobs de CI corren y pasan en verde | — | dev-mobile-rn + dev-backend-java | CR-H0.11 | T18 |
| T20 | ☐ | Configurar protección de rama en `main` requiriendo los 3 jobs (acción manual fuera del repo, en GitHub) | — | arquitecto | — | T19 |
| T21 | ◐ | Verificación final de la Definición de Hecho (DoD) de H0 contra todos los `CR-H0.x`. **Hallazgo:** el paso "Test (with coverage)" del job `mobile` en `ci.yml` tenía un `--` duplicado (bug de `pnpm --filter ... run test -- --coverage` con pnpm 12.5.1) que hacía que Jest recibiera argumentos inválidos y saliera en verde SIN correr ninguna prueba (`No tests found, exiting with code 0`). **Corregido** a `pnpm --filter @fitapp/mobile test --coverage` y reverificado localmente (26/26 tests, cobertura real). Queda ◐ en vez de ☑ porque falta que el usuario confirme el fix en un PR real (CR-H0.11 sigue sin verificación remota posible desde este entorno). También se reportó que el commit `61e965b` no sigue Conventional Commits en inglés (Art. 8.3) — pendiente de decisión del usuario. | — | qa-pruebas | CR-H0.1–CR-H0.13 | T19 |
